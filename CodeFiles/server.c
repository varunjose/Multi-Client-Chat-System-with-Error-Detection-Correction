...
void handle_client_message(connection_info clients[], int sender, FILE *file) {
    int read_size;
  message msg;
  ...
  else
  {
    ...
    switch (msg.type) {
        ...
        case PRIVATE_MESSAGE:
            ...
            fprintf(file, "(Private) %s to %s: %s\n", clients[sender].username, msg.username, msg.data);
            break;
  ...
  while (true) {
        ...
        for (i = 0; i < MAX_CLIENTS; i++) {
            if (clients[i].socket > 0 && FD_ISSET(clients[i].socket, &file_descriptors)) {
                handle_client_message(clients, i, file); 
            }
        }
    }
